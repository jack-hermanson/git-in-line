...
router.post(
    "/",
    auth,
    async (req: AuthRequest<NewPrRequest>, res: Response) => {
        try {
            if (!(await validateRequest(newPrSchema, req, res))) return;

            const requestBody: NewPrRequest = req.body;
            const pullRequest = await PullRequestService.create(
                requestBody,
                req.account,
                res
            );
...
router.put(
    "/:id",
    auth,
    async (req: AuthRequest<EditPrRequest & { id: number }>, res: Response) => {
        try {
            ...
            const editedPr = await PullRequestService.edit(
                requestBody,
                id,
                res
            );
